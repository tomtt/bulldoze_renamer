...
RSpec.describe CrudeRenamer::FileFinder do
  ...
  describe "extract_directories" do
    it "is empty for no files" do
      expect(CrudeRenamer::FileFinder.extract_directories([])).to be_empty
    end

    it "has the directory of the single file" do
      expect(CrudeRenamer::FileFinder.extract_directories(['a/b'])).to eq(['a'])
    end

    it "has the same directory of the multiple file once" do
      expect(CrudeRenamer::FileFinder.extract_directories(['d/f1', 'd/f2'])).to eq(['d'])
      expect(CrudeRenamer::FileFinder.extract_directories(['foo/bar', 'foo/baz'])).to eq(['foo'])
    end

    it "has and entry for every subdirectory of a single file" do
      expect(CrudeRenamer::FileFinder.extract_directories(['one/two/three/file'])).
    ...
    it "only does not traverse directories multiple times" do
      ...
      result = CrudeRenamer::FileFinder.extract_directories(
        [
          'one/file1',
          'one/file2',
          'one/file3'
